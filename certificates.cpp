 /*
 NetworkClientSecure 对远程服务器（如 github、smtp）的连接进行加密
 若要可选地验证远程服务器身份（防止中间人攻击），
 应用需要检查其公钥证书。
 使用 openssl 工具获取远程服务器的公钥证书，例如：
 ...
 将最后列出的证书（通常是根 CA 证书）复制并粘贴到下方相关常量中。
 要禁用证书检查（NetworkClientSecure），请将相关常量留空，和/或
 在网页的 Access Settings / Authentication settings 中将 Use Secure 设为关闭

 FTP 连接为明文，未实现 FTPS。


 要将应用设为 HTTPS 服务器，需要服务器私钥和公钥证书
 使用 openssl 工具创建密钥和证书
 在 Windows 上，将下方命令粘贴到命令提示符（cmd）窗口中
