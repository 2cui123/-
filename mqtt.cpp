...
#endif 
char mqtt_broker[MAX_HOST_LEN] = "";         // 要连接的 MQTT 服务器 IP
char mqtt_port[5] = "";                      // 要连接的 MQTT 服务器端口
char mqtt_user[MAX_HOST_LEN] = "";           // MQTT 服务器用户名
char mqtt_user_Pass[MAX_PWD_LEN] = "";       // MQTT 服务器密码
char mqtt_topic_prefix[FILE_NAME_LEN / 2] = "";  // 发布负载的 MQTT 主题前缀
char image_topic[FILE_NAME_LEN] = "";  // 发布图像负载的 MQTT 主题
...
bool mqtt_active = false;         // 是否启用
bool mqttRunning = false;         // MQTT 任务是否运行中
bool mqttConnected = false;       // 是否已连接到 broker
...
void mqttPublish(const char* payload) {
  if (!strlen(mqtt_topic_prefix)) return; // 是否在加载配置前调用？
...
static void mqtt_disconnected_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
  ...
  mqttConnected = false;
  //xTaskNotifyGive(mqttTaskHandle); // 解除任务阻塞
}
...
static void mqtt_error_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
  ...
  if (event->error_handle->error_type == MQTT_ERROR_TYPE_TCP_TRANSPORT) {
    // log_error_if_nonzero("来自 esp-tls 的错误", event->error_handle->esp_tls_last_esp_err);
    // log_error_if_nonzero("来自 tls 栈的错误", event->error_handle->esp_tls_stack_err);
    // log_error_if_nonzero("传输层 socket errno", event->error_handle->esp_transport_sock_errno);
...
#endif // 结束 INCLUDE_HASIO
#endif // 结束 INCLUDE_MQTT
