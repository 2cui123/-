...
typedef struct {
    ...
    uint32_t pwmb_gpio_num; /*!< BDC Motor PWM B gpio number (0 = PWM+DIR mode) */
    uint32_t pwm_freq_hz;   /*!< PWM frequency, in Hz */
    int dir_gpio_num;       /*!< Direction gpio when pwmb is 0 (HIGH=fwd, LOW=rev) */
} bdc_motor_config_t;
...
{
    bdc_motor_mcpwm_obj *mcpwm_motor = __containerof(motor, bdc_motor_mcpwm_obj, base);
    if (mcpwm_motor->dir_gpio_num > 0) {
        digitalWrite(mcpwm_motor->dir_gpio_num, HIGH);
        return mcpwm_generator_set_force_level(mcpwm_motor->gena, -1, true);
    }
    ESP_RETURN_ON_ERROR(mcpwm_generator_set_force_level(mcpwm_motor->gena, -1, true), TAG, "disable force level for gena failed");
    if (!mcpwm_motor->genb) return ESP_OK;
...
static esp_err_t bdc_motor_mcpwm_reverse(bdc_motor_t *motor)
{
    bdc_motor_mcpwm_obj *mcpwm_motor = __containerof(motor, bdc_motor_mcpwm_obj, base);
    if (mcpwm_motor->dir_gpio_num > 0) {
        digitalWrite(mcpwm_motor->dir_gpio_num, LOW);
        return mcpwm_generator_set_force_level(mcpwm_motor->gena, -1, true);
    }
    if (!mcpwm_motor->genb) return ESP_OK;
...
{
    bdc_motor_mcpwm_obj *mcpwm_motor = __containerof(motor, bdc_motor_mcpwm_obj, base);
    if (mcpwm_motor->dir_gpio_num > 0) return bdc_motor_mcpwm_coast(motor);
...
static esp_err_t bdc_motor_mcpwm_del(bdc_motor_t *motor)
{
    ...
    if (mcpwm_motor->genb) mcpwm_del_generator(mcpwm_motor->genb);
...
static esp_err_t bdc_motor_new_mcpwm_device(const bdc_motor_config_t *motor_config, const bdc_motor_mcpwm_config_t *mcpwm_config, bdc_motor_handle_t *ret_motor)
{
    ...
    if (mcpwm_motor->dir_gpio_num > 0) {
        pinMode(mcpwm_motor->dir_gpio_num, OUTPUT);
        digitalWrite(mcpwm_motor->dir_gpio_num, LOW);
    }
    ...
    if (motor_config->pwmb_gpio_num > 0) {
...
// pwmDirMode: true = pwmAgpio speed PWM + dirGpio direction; false = dual-PWM H-bridge
static bool prepBDCmotor(int groupId, int motorId, int pwmAgpio, int dirOrRevGpio, bool pwmDirMode = true) {
  bdc_motor_config_t BDCmotorConfig = {
      .pwma_gpio_num = (uint32_t)pwmAgpio,
      .pwmb_gpio_num = pwmDirMode ? 0 : (uint32_t)dirOrRevGpio,
      .pwm_freq_hz = (uint32_t)pwmFreq,
      .dir_gpio_num = pwmDirMode ? dirOrRevGpio : 0,
  };
  ...
  if (res == ESP_OK) {
    if (pwmDirMode) LOG_INF("Motor %d: PWM pin %d, DIR pin %d", motorId, pwmAgpio, dirOrRevGpio);
    else LOG_INF("Initialising MCPWM unit %d, motor %d, using pins %d, %d", groupId, motorId, pwmAgpio, dirOrRevGpio);
...
void motorSpeed(int speedVal, bool leftMotor) {
  ...
  int motorId = leftMotor ? 0 : 1;
  if (!BDCmotor[motorId]) {
    static uint32_t lastWarnMs = 0;
    uint32_t now = millis();
    if (now - lastWarnMs > 5000) {
      lastWarnMs = now;
      LOG_WRN("Motor %d not init (RCactive=%d, motorFwdPin=%d, motorRevPin=%d)", motorId, RCactive, motorFwdPin, motorRevPin);
    }
    return;
  }
...
void prepMotors() {
  ...
  aux = true;
#endif
  if (RCactive && !(useUart && !aux)) useBDC = true;
  if (!useBDC) return;

  // tail motor defaults (PWM=1, DIR=2) if RC enabled but pins not saved on SD
  if (motorFwdPin <= 0) motorFwdPin = 1;
  if (motorRevPin <= 0) motorRevPin = 2;

  if (motorInitDone) return;

  if (motorFwdPin > 0) {
    if (prepBDCmotor(0, 0, motorFwdPin, motorRevPin, true)) motorInitDone = true;
    else LOG_ERR("Motor init failed: PWM pin %d, DIR pin %d", motorFwdPin, motorRevPin);
    if (trackSteer && motorFwdPinR > 0) prepBDCmotor(0, 1, motorFwdPinR, motorRevPinR, false);
